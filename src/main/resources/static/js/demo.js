...
const app = async () => {
    ...
    // }).catch(err => console.error(err));
    //
    // tronWeb.trx.getAccount(tronWeb.defaultAddress.hex).then(accountInfo => {
    ...
    const myAddress = 'TNx43yPrp5mitcnTwUmcVgpuRK7GniAhJT';
    //trondice
    // const contractAddress = 'TMYcx6eoRXnePKT1jVn25ZNeMNJ6828HWk';
    // const contractAddress = 'TG1kDMhM4gChzDSQJLq3ZGV8kznBfNZirj';
    const contractAddress = 'TV7PkRBUPwzreB7cgX9rMe7mq3uVCUmjeg';
    ...
    tronWeb.trx.getContract(contractAddress).then(contract => {
        ...
        console.log('- Contract Address: TMYcx6eoRXnePKT1jVn25ZNeMNJ6828HWk');
        console.log('- Origin Address:', contract.origin_address);
        console.log('- Bytecode:', contract.bytecode);
        console.log('- ABI:\n' + JSON.stringify(contract.abi, null, 2), '\n');
        console.groupEnd();
        // contract = tronWeb.contract({abi:contract.abi});
        // contract.bet(100);
    }).catch(err => console.error(err));
    ...
    //         "inputs": [],
    //         "name": "getLast",
    //         "outputs": [{"name": "a", "type": "int256"}, {"name": "b", "type": "int256"}, {"name": "result", "type": "int256"}],
    //         "payable": false,
    ...
    //     }, {
    //         "anonymous": false,
    //         "inputs": [{"indexed": false, "name": "a", "type": "int256"}, {"indexed": false, "name": "b", "type": "int256"}, {"indexed": false, "name": "result", "type": "int256"}],
    //         "name": "Message",
    //         "type": "event"
    //     }],
    ...
    newContract = await tronWeb.contract().at(contractAddress);
    console.log(newContract);
    newContract.bet(50).send({
        callValue: window.tronWeb.toSun(),
        shouldPollResponse: true,
        feeLimit:10000
    }).catch(function (t) {
        console.log(t)
    });
    newContract.last_completed_migration().call().then(result => {
        console.log(result);
    }).catch(err => console.error(err));
